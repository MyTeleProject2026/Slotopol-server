...
func ApiClubProfileSet(c *gin.Context) {
	...
	n,err:=XormStorage.Where("club_id=?",p.ClubID).AllCols().Update(&p); if err!=nil { Ret500(c,0,err); return }; if n==0 { if _,err=XormStorage.InsertOne(&p); err!=nil { Ret500(c,0,err); return } }; RetOk(c,p)
}
...
func ApiClubProfileGet(c *gin.Context) {
	...
	if err:=ensureClubProfiles(); err!=nil { Ret500(c,0,err); return }; var p ClubProfile; has,err:=XormStorage.ID(arg.CID).Get(&p); if err!=nil { Ret500(c,0,err); return }; if !has { Ret404(c,0,ErrNoClub); return }; RetOk(c,p)
}
...
func ApiCountryGameProfileSet(c *gin.Context) {
	...
	p.CountryCode=strings.ToUpper(strings.TrimSpace(p.CountryCode)); p.Currency=strings.ToUpper(strings.TrimSpace(p.Currency)); var old CountryGameProfile; has,err:=XormStorage.Where("country_code=? AND currency=?",p.CountryCode,p.Currency).Get(&old); if err!=nil { Ret500(c,0,err); return }; if has { p.ID=old.ID; _,err=XormStorage.ID(old.ID).AllCols().Update(&p) } else { _,err=XormStorage.InsertOne(&p) }; if err!=nil { Ret500(c,0,err); return }; RetOk(c,p)
}

...
var operationSchemaOnce sync.Once
var operationSchemaErr error

// EnsureOperationalLogsSchema lazily initializes the TiDB/MySQL-compatible
// operational log table exactly once. This keeps schema work out of every
// request while still requiring no manual SQL from the operator.
func EnsureOperationalLogsSchema() error {
    operationSchemaOnce.Do(func() {
        if XormStorage == nil { operationSchemaErr = fmt.Errorf("club database storage is not initialized"); return }
        operationSchemaErr = XormStorage.Sync(new(OperationalLog))
    })
    return operationSchemaErr
}
...
func writeOperationalLog(e OperationalLog) { if XormStorage!=nil { if err:=EnsureOperationalLogsSchema(); err!=nil { e.Level="ERROR"; e.EventType="operations.persistence_error"; e.Error=strings.TrimSpace(strings.Join([]string{e.Error,err.Error()},": ")) } else if _,err:=XormStorage.InsertOne(&e); err!=nil { e.Level="ERROR"; e.EventType="operations.persistence_error"; e.Error=strings.TrimSpace(strings.Join([]string{e.Error,err.Error()},": ")) } }; publishOperationalLog(e) }
...
func ApiAdminOperationsSummary(c *gin.Context) { _,al:=GetAdmin(c,0); if al&ALadmin==0 { Ret403(c,0,ErrNoAccess); return }; if err:=EnsureOperationalLogsSchema(); err!=nil { Ret500(c,0,err); return }
    total,err:=XormStorage.Count(new(OperationalLog)); if err!=nil { Ret500(c,0,err); return }; errors,err:=XormStorage.Where("level=?","ERROR").Count(new(OperationalLog)); if err!=nil { Ret500(c,0,err); return }; warnings,err:=XormStorage.Where("level=?","WARN").Count(new(OperationalLog)); if err!=nil { Ret500(c,0,err); return }; var latest OperationalLog; has,err:=XormStorage.Desc("id").Get(&latest); if err!=nil { Ret500(c,0,err); return }; var last any; if has { last=latest }; RetOk(c,gin.H{"total":total,"errors":errors,"warnings":warnings,"latest":last}) }
func ApiAdminOperationsLogs(c *gin.Context) { _,al:=GetAdmin(c,0); if al&ALadmin==0 { Ret403(c,0,ErrNoAccess); return }; if err:=EnsureOperationalLogsSchema(); err!=nil { Ret500(c,0,err); return }; limit:=100; if v,err:=strconv.Atoi(c.Query("limit")); err==nil && v>0 && v<=500 { limit=v }; q:=XormStorage.Desc("id"); if level:=strings.TrimSpace(c.Query("level")); level!="" { q=q.Where("level=?",strings.ToUpper(level)) }; if et:=strings.TrimSpace(c.Query("event_type")); et!="" { q=q.Where("event_type=?",et) }; var rows []OperationalLog; if err:=q.Limit(limit).Find(&rows); err!=nil { Ret500(c,0,err); return }; RetOk(c,gin.H{"events":rows}) }
func ApiAdminOperationsStream(c *gin.Context) { _,al:=GetAdmin(c,0); if al&ALadmin==0 { Ret403(c,0,ErrNoAccess); return }; if err:=EnsureOperationalLogsSchema(); err!=nil { Ret500(c,0,err); return }; c.Header("Content-Type","text/event-stream"); c.Header("Cache-Control","no-cache"); c.Header("Connection","keep-alive"); c.Header("X-Accel-Buffering","no")

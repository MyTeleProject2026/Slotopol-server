...
func ApiAllocationCreate(c *gin.Context) {
	...
	recordAdminAudit(c, arg.ClubID, "treasury.allocation.create", "allocation", txID)
	RetOk(c, alloc)
}
...
func ApiAllocationApprove(c *gin.Context) {
	...
	if has, err := XormStorage.ID(arg.ID).Get(&alloc); err != nil || !has { Ret404(c, 0, errors.New("allocation not found")); return }
	...
	session := XormStorage.NewSession(); defer session.Close()
	if err := session.Begin(); err != nil { Ret500(c, 0, err); return }
	...
	recordAdminAudit(c, alloc.ClubID, "treasury.allocation.approve", "allocation", alloc.TransactionID)
	Ret204(c)
}
...
func ApiAllocationList(c *gin.Context) {
	ensureAllocationTable()
	admin, al := MustAdmin(c, 0)
	if admin == nil || al&ALadmin == 0 { Ret403(c, 0, ErrNoAccess); return }
	var allocs []Allocation
	if err := XormStorage.Desc("created_at").Find(&allocs); err != nil { Ret500(c, 0, err); return }

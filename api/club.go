...
func ApiClubJpfund(c *gin.Context) {
	...
	// Persist fund directly to the club row.
	// SqlBank does not provide a Fund method in the current repository.
	if _, err = XormStorage.ID(arg.CID).Cols("fund").Update(&ClubData{Fund: arg.Fund}); err != nil {

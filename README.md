...
This server now supports uploading, storing, and serving game images (banners, thumbnails, logos, etc.) via Cloudinary. All image assets are stored in the cloud, reducing server load and enabling CDN delivery.

Cloudinary API Endpoints

Method Endpoint Description
POST /cloudinary/upload Upload an image (multipart/form-data)
GET /cloudinary/images List uploaded images (optional ?folder=...)
DELETE /cloudinary/image Delete image by public_id (JSON body)

Upload example:

```bash
curl -X POST http://localhost:8080/cloudinary/upload \
...
```bash
curl -X GET "http://localhost:8080/cloudinary/images?folder=games" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```bash
curl -X DELETE http://localhost:8080/cloudinary/image \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"public_id":"your_public_id"}'
```
...
```cmd
git clone https://github.com/MyTeleProject2026/Slotopol-server.git
cd server
...
Endpoint receives alias identifier to game that represents as concatenation of provider name and game name with slash. For example, NetEnt/Tiki Wonders. Whole list of all supported games can be obtained by list command. Identifier turns to lowercase without spaces.
...
To deploy this server on Render.com:

1. Push your repository to GitHub.
2. On Render, create a Web Service and connect your repo.
3. Set the following:
   · Environment: Go
   · Build Command: go build -tags netgo -ldflags '-s -w' -o slotopol-server
   · Start Command: ./slotopol-server -v web -f ./appdata/slot-app.yaml
4. In the Environment Variables section, add all required variables (database, Cloudinary, JWT keys). See the .env.example file for reference.
5. Deploy.

Environment Variables

The server uses environment variables (prefixed SLOTOPOL_) for configuration. The most important ones are:

Variable Description Required
SLOTOPOL_CLUBDSN Database DSN for club data (e.g., user:pass@tcp(host:port)/db) Yes
SLOTOPOL_SPINDSN Database DSN for spin logs Yes
SLOTOPOL_ACCESS_KEY JWT access key (generate your own) Yes
SLOTOPOL_REFRESH_KEY JWT refresh key Yes
SLOTOPOL_CLOUDINARY_CLOUD_NAME Cloudinary cloud name Yes (if using Cloudinary)
SLOTOPOL_CLOUDINARY_API_KEY Cloudinary API key Yes (if using Cloudinary)
SLOTOPOL_CLOUDINARY_API_SECRET Cloudinary API secret Yes (if using Cloudinary)
SLOTOPOL_CLOUDINARY_UPLOAD_FOLDER Base upload folder (default: slotopol) No
SLOTOPOL_PORT HTTP listen port (default: 8080) No
SLOTOPOL_DBDRIVER Database driver (sqlite3, mysql, postgres) No

A full list is available in the .env.example file.
